...
"""
def convert_to_vol_ave_confined_SML(input_4d_array,MLD_layers,cross_area_idx,
                                    cross_area,
                                    layer_depth,
                                    dz,
                                    dx=1e3,
                                    ...
                                    cross=np.arange(300)):
    input_along_int=input_4d_array.sum(axis=2)*dx
    input_3d=np.zeros((len(run_date),len(layer_depth),len(cross)))
    for i in range(len(run_date)):
        for xx in range(len(cross)):
            if MLD_layers[i]<cross_area_idx[xx]:
                input_3d[i,0:int(MLD_layers[i]),xx]=input_along_int[i,0:int(MLD_layers[i]),xx]
            else:
                input_3d[i,0:int(cross_area_idx[xx]),xx]=input_along_int[i,0:int(cross_area_idx[xx]),xx]
    
    input_dep_int=input_3d.sum(axis=1)*dz
    input_dep_ave=input_dep_int/cross_area
    input_vol_ave=input_dep_ave.mean(axis=1)
    return input_vol_ave

def convert_3d_to_vol_ave_confined_SML(input_3d_array,MLD_layers,cross_area_idx,
                                    cross_area,
                                    layer_depth,
                                    dz,
                                    dx=1e3,
                                    run_date=np.arange(120),
                                    cross=np.arange(300)):
    input_along_int=input_3d_array*dx
    input_3d=np.zeros((len(run_date),len(layer_depth),len(cross)))
    for i in range(len(run_date)):
        for xx in range(len(cross)):
            if MLD_layers[i]<cross_area_idx[xx]:
...
def convert_to_vol_ave_confined_SML(input_4d_array,MLD_layers,
                                    layer_depth,
                                    dz,
                                    dx=1e3,
                                    run_date=np.arange(120),
    ...
    input_3d=np.zeros((len(run_date),len(layer_depth),len(cross)))
    for i in range(len(run_date)):
        input_3d[i,0:int(MLD_layers[i].values/5-1),:]=input_along_int[i,0:int(MLD_layers[i].values/5-1),:]
        
    cross_area_idx_MLD=np.zeros((len(run_date),len(cross)))
    for i in range(len(run_date)):
        for xx in range(len(cross)):
            cross_area_idx_MLD[i,xx]=np.count_nonzero(np.nan_to_num(input_3d[i,:,xx])) 
    cross_area_MLD=cross_area_idx_MLD*dz*250e3

    input_dep_int=input_3d.sum(axis=1)*dz
    input_dep_ave=input_dep_int/cross_area_MLD
    input_vol_ave=input_dep_ave.mean(axis=1)

    return input_vol_ave


def convert_3d_to_vol_ave_confined_SML(input_3d_array,MLD_layers,
                                    layer_depth,
                                    dz,
                                    dx=1e3,
                                    run_date=np.arange(120),
    ...
    input_dep_ave=input_dep_int/cross_area_MLD
    # input_vol_ave=input_dep_ave.mean(axis=1)
    # input_vol_ave=input_dep_ave.mean(skipna='Ture',axis=1)
    input_vol_ave=np.nanmean(input_dep_ave,axis=1)

    return input_vol_ave

def convert_4d_var_to_domain_vol_ave(input_4d_array,
                                    cross_area,
                                    dz,
                                    dx=1e3):
    input_dep_int=input_4d_array.sum(axis=2).sum(axis=1)*dx*dz
    input_dep_ave=input_dep_int/cross_area
    input_vol_ave=input_dep_ave.mean(axis=1)
    return input_vol_ave

def convert_3d_var_to_domain_vol_ave(input_3d_array,
                                    cross_area,
                                    dz,
                                    dx=1e3):
    input_dep_int=input_3d_array.sum(axis=1)*dz*dx
    # input_dep_int=input_3d_array.sum(axis=1)*dz*dx*250
    input_dep_ave=input_dep_int/cross_area
    input_vol_ave=input_dep_ave.mean(axis=1)
    return input_vol_ave
